...
菜单栏悬停卡片（停在菜单栏歌词上 0.3s 弹出，挂在菜单栏下方、右缘与歌词右缘对齐）：
...
- 播放、暂停、上一首、下一首；播放模式：顺序（首尾循环：末首的下一首回第一首、首首的上一首到末首）、单曲循环、随机（偏好持久化）。
- 写入元数据（支持批量）：曲目行右键「写入元数据…」写单首，文件菜单「为当前列表写入元数据…」批量写，运行中可随时停止。把歌名、歌手、专辑、封面和歌词写进音频文件本身，方便在其他播放器和设备上正确显示。仅 mp3 / flac / m4a；正在播放的那首会跳过。只补空着的字段，已有内容不会被覆盖；每个文件写入前都完整备份到 `~/Library/Application Support/LocalLrcPlayer/MetadataBackups/`，写入用 ffmpeg 流拷贝不重新编码。备份不会无限堆积：同一首歌只留最新一份，超过 14 天或总量超 1 GB 的旧备份在任务结束后自动清理，文件菜单「元数据备份…」可查看占用并一键删除。歌词真相仍是音频旁的 `.lrc`，内嵌歌词只为方便带去别的设备。
- 自建播放列表：点列表顶栏的标题（「歌曲 · N ⌄」）可在「全部」与自建列表间切换，并新建 / 重命名 / 删除；曲目行右键「加入播放列表 ▸」按勾选加入或移出，查看自建列表时可「从当前列表移除」；文件菜单「新建播放列表…」（⌘N）。搜索与播放队列都在当前列表内生效；重启后仍停留在上次查看的列表。删除列表不会删除任何歌曲文件。
- 播放队列：曲目行右键（或「播放」菜单对选中行）选「下一首播放」插到队首、「稍后播放」追加队尾；队列非空时「下一首」和顺序/随机模式自动播完都先按队列走，单曲循环时自动播完仍循环本曲。行副标题末尾显示「队列 N」，右键可清空。队列不持久化，退出即空。
...
- 音乐文件夹自动感知变化：往文件夹里增删、改名音乐或 `.lrc` 后约 2 秒自动增量同步并刷新列表，不必 ⌘R；同步在后台进行，不卡界面。文件夹本身被移走或外接盘拔掉时不会误删索引。
...
- 设置（⌘,）：外观（跟随系统/浅色/深色）、音乐库（添加/移除文件夹）、歌词 Cookie 与下载、菜单栏歌词。
...
- 播放里程碑：某首歌的有效播放次数达到 10/50/100/300/500/1000 时，播完当前这首后弹出庆祝面板（大封面 + 滚动计数 + 首次播放至今的天数），底色取自封面主色。有效播放沿用 Last.fm 规则：曲长超过 30 秒且实际听满一半或 4 分钟（取较小者），跳过不计。里程碑从功能上线后重新起算。
- 往年今日：启动时回顾同一天你在听什么（按 12/6/3/1 个月依次取第一个有记录的跨度，数据攒够一年后自动变成真正的「往年今日」）。每天最多一次，那天没记录就不打扰。
- 上面两项都可在 ⌘, 设置 → 播放里程碑 中关闭，弹窗里也有「不再提醒」。
...
- 菜单栏悬停卡片：鼠标停在菜单栏歌词上 0.3s 弹出正在播放卡片（封面、歌名/歌手、当前歌词、可拖动进度条、上一首/播放暂停/下一首、播放模式、音量），移开自动收起。卡片是无箭头的圆角面板，挂在菜单栏下方、右缘与歌词右缘对齐，歌词换行导致宽度变化时位置不动。多显示器下只有当前聚焦那块屏会弹出。
- 标准 macOS 菜单栏与常用快捷键（⌘Q、空格播放/暂停等）。
- 无障碍：所有图标按钮、进度条/音量滑杆、菜单栏歌词状态项都有 VoiceOver 标签，进度条念「已播 / 总长」；打开系统「键盘导航」后可用 Tab 聚焦进度条并用方向键跳转。
- 排查问题：帮助 → 导出诊断信息… 会生成一份纯文本（环境、设置、音乐文件夹可达性、播放态、最近歌词下载记录、本次运行日志），不含 Cookie 值。也可在 Console.app 或终端按 subsystem 查看日志：`log show --predicate 'subsystem == "local.lrc.player.v2"' --last 1h`。播放失败（文件损坏、格式不支持）会在状态栏提示，不再静默无声。
- 系统「正在播放」集成：键盘媒体键（F7/F8/F9）、AirPods 捏合、控制中心与菜单栏的正在播放卡片都能控制播放/暂停、上一首/下一首和拖进度；卡片显示歌名、歌手、封面与进度。媒体键的播放只继续当前曲目，不会跳到列表选中行。
...
Sources/LocalLrcPlayer/
  ...
  MilestoneCelebration.swift      庆祝面板（里程碑/往年今日共用）
  OnThisDayMemory.swift           往年今日的日期回推与文案
  ArtworkColor.swift              封面取色（氛围背景与庆祝面板共用）
